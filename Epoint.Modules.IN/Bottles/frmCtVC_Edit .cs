...
namespace Epoint.Modules.IN
{
    ...
    public partial class frmCtVC_Edit : frmVoucher_Edit
    {
        ...
        public static string GetTonCuoi(DataRow drEditCt, ref double dbTonCuoi)
        {
            ...
            DataTable dt = SQLExec.ExecuteReturnDt("Sp_VC_GetTonCuoi", ht, CommandType.StoredProcedure);
            DataRow dr = dt.Rows[0];

            dbTonCuoi = Convert.ToDouble(dr["Ton_Cuoi"]);

            return (string)dr["Ten_Vt"] + ": " + Convert.ToDouble(dr["Ton_Cuoi"]).ToString("N0") + " " + (string)dr["Dvt"];
        }
        #endregion
        ...
        void dgvEditCt_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Common.Inlist(strColumnName, "SO_LUONG9,GIA_NT9,TIEN_NT9,TIEN"))
            {
                    ...
                    (string)drDmCt["Nh_Ct"] == "2" && strColumnName == "SO_LUONG9")
                {
                    ...
                    double dbTon_Cuoi = 0;
                    GetTonCuoi(drCurrent, ref dbTon_Cuoi);
        ...
        void dgvEditCt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Space)
                if (dgvEditCt1.CurrentCell.OwningColumn.DataPropertyName == "DVT")
                {
                    ...
                    DataRow drDmVt = DataTool.SQLGetDataRowByID("LIVATTUVC", "Ma_Vt", strMa_Vt);
